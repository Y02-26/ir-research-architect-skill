...
2. List several possible dependent variables / outcomes.
3. Give two or three research-question versions.
...
Do not propose a full causal mechanism before the outcome and scope are fixed.
...
```text
先不要急着写假设。这个阶段应该先列出可能的待解释结果 Y，再决定案例、时间范围和证据来源。
```
